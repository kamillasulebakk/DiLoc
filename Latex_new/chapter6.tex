...
% In this section we will see how a Diloc would have perfomed the same task of localizing simple current dipoles from EEG recorings, if it were designed as a convolutional neural network. The convolutional neural network (CNN) is a sligtly more complicated network than the one presented in the previous section, however, it is still classified as a type of feed forward neural network.
%
% \subsection{Purpose...}
% The purpose of testing how a convolutional neural network will perform the same task is to see if the use of spatial information in images will help the network analyze EEG recordings better and thus give more accurate predictions for the localization of the sorces generating the signal.
%
% \subsection{Data set}
% Convoltuinal Neural Networks takes images as input data, and learn several local features in order to produce final predictions. Starting from the orignal dataset presentesd in Chapter 4, image data were produced by the use of interpolation. The tequniqe of interpolation is commonly used to insert an intermediate value between two other values. By interpolating one dimensional EEG data we can convert orignial electrode positions to a regular 2D grid, creating image data, where spatial structures as conserved.
%
%
% This image data works as the new data set for the convolutional network. Data consist of three dimensional matrix, and before beeing fed into the network it needs to be reshaped so that it gets the shape of one sigle coloumn.
