...
\rednote{The model it self? Do i mean activation function here or the whole network?}
\rednote{quote}
...
The first element is the dataset $\mathcal{D} = (X, Y)$. $X$, commonly is referred to as the \emph{design matrix}. and we aim to find a model that gives $Y$ as function of $X$. \rednote{fix}.
\rednote{X sett av alle sett av inputverdier i datasettet. x er et sett av alle inputverdier i en sample (hele inputtsettet).}
Next, we have the model $f(x; \boldsymbol{\theta})$ itself which is a function $f : \mathcal X \to \mathcal Y$ used to predict an output $y \in \mathcal Y$ from an input $x \in \mathcal X$ using the \emph{model parameters} $\boldsymbol{\theta}$. \rednote{$\mathbb{R}^{231}$ til $\mathbb{R}^{3}$}
...
Training the model is done by adjusting the model parameters in order to minimize the output of the cost function. The cost function gets high values when the output does not fit with the output wantet. Goal to find parameters that minimize the cost. \rednote{fix}
...
We will consider fully connected feed-forward \rednote{flere spesifikasjoner???} neural networks whose nodes are grouped into \emph{layers} with the output of one layer serving as the input for the next. \rednote{Hver node i et layer sender sine tall videre til hver node i det neste layer. Ingen tall går bakover... Antall noder i layer nummer l er $N_l$}. The activation of the $i$-th node in layer $l$, $a_i^{(l)}$ is the weigted sum
\begin{equation}
  z_i^{(l)} = \sum_{j=1}^{N_{l-1}} w_{ij}^{(l)} a_{j}^{(l-1)} + b_i^{(l)}
\end{equation}
fed through an \emph{activation function} which we will soon come back to. \rednote{fix}. \rednote{bias b}. The activation of the final layer is the outputs of the model. Typically, we call the first layer of neural networks the \emph{input layer}, the final layer of the model \emph{output layer} and the layers in between \emph{hidden layers}. In figure \ref{fig:NN_basic_architecture} we have illustrated the basic architecture of neural networks. Here nodes are depiced as circular shapes, while arrows indicate connections between the nodes. \rednote{maybe remove first figure? wrong notation}.
...
\rednote{Here I need to explain pros with ReLU in fist layer. Include the problem of dead neurons.}
...
Tanh is continuous and differentiable at all points, that for reasons we will come back to is making it a suitable activation function for neural networks \rednote{Where do I come back to this?}. Its mathematical representation is as follows:
