...
def main(
    ...
    MSE_x1 = np.zeros(N_epochs)
    MSE_y1 = np.zeros(N_epochs)
    MSE_z1 = np.zeros(N_epochs)
    MSE_A1 = np.zeros(N_epochs)

    if N_dipoles == 2:
        MSE_x2 = np.zeros(N_epochs)
        MSE_y2 = np.zeros(N_epochs)
        MSE_z2 = np.zeros(N_epochs)
        MSE_A2 = np.zeros(N_epochs)
    ...
    for epoch in range(N_epochs):
        ...
        for i, (signal, target) in enumerate(data_loader_test):
            ...
            MSE_x1[epoch] = np.mean((target_[:][0] - pred_[:][0]) ** 2)
            MSE_y1[epoch] = np.mean((target_[:][1] - pred_[:][1]) ** 2)
            MSE_z1[epoch] = np.mean((target_[:][2] - pred_[:][2]) ** 2)
            MSE_A1[epoch] = np.mean((target_[:][3] - pred_[:][3]) ** 2)

            if N_dipoles == 2:
                MSE_x2[epoch] = np.mean((target_[:][4] - pred_[:][4]) ** 2)
                MSE_y2[epoch] = np.mean((target_[:][5] - pred_[:][5]) ** 2)
                MSE_z2[epoch] = np.mean((target_[:][6] - pred_[:][6]) ** 2)
                MSE_A2[epoch] = np.mean((target_[:][7] - pred_[:][7]) ** 2)
