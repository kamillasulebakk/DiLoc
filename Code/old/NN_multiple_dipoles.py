...
class EEGDataset(torch.utils.data.Dataset):
    def __init__(self, train_test: str, name: str, N_samples: int, N_dipoles: int):
        ...
        eeg, pos_list = produce_and_load_eeg_data.load_data(N_samples, 'multiple_dipoles', num_dipoles = N_dipoles)
        mean, std_dev = produce_and_load_eeg_data.load_mean_std(N_samples, f'multiple_dipoles_{N_dipoles}')
...
def main(name: str, N_samples = 10_000, N_dipoles = 1, N_epochs = 2000, noise_pct = 10):
    ...
    with open(f'results/01.feb/restult_multiple_dipoles_{N_dipoles}_NN_{N_samples}_l1.txt', 'w') as f:
    ...
    for epoch in range(N_epochs):
        ...
        with open(f'results/01.feb/restult_multiple_dipoles_{N_dipoles}_NN_{N_samples}_l1.txt', 'a') as f:
            f.write(f'epoch {epoch:2d}, train loss: {train_loss[epoch]:9.3f}')
            f.write(f', test loss: {test_loss[epoch]:9.3f} \n')

    plot_MSE_NN(train_loss, test_loss, f'multiple_dipoles_{N_dipoles}_noise{noise_pct}_{N_samples}_l1', 'TanH', batch_size, N_epochs, name = name)
...
if __name__ == '__main__':
    ...
    net = main('multiple_dipoles', N_samples, N_dipoles, N_epochs, noise_pct)

    PATH = f'trained_models/NN_multiple_diples_{N_dipoles}_noise{noise_pct}_{N_epochs}_{N_samples}_l1.pt'
    torch.save(net, PATH)
