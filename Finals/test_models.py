...
from utils import MSE, MAE, MAPE
...
class TestResults:
    ...
    def _MAPE(self, dimension: int):
        return MAPE(self._predictions[:, dimension], self._targets[:, dimension])
    ...
    @property
    def MAPE_amplitude(self):
        return self._MAPE(3)

    @property
    def MAPE_radius(self):
        return self._MAPE(4)

    @property
    def MAPE_position(self):
        # TODO: Verify that this logic is correct
        return (self.MAPE_x + self.MAPE_y + self.MAPE_z)/3
...
def generate_test_results(predictions: np.ndarray, targets: np.ndarray):
    ...
    d = {
        ...
        'MAPE_x': results.MAPE_x,
        'MAPE_y': results.MAPE_y,
        'MAPE_z': results.MAPE_z,
        'MAPE_amplitude': results.MAPE_amplitude,
        'MAPE_radius': results.MAPE_radius,
        'MAPE_position': results.MAPE_position,
        'MSE_x': results.MSE_x,
        'MSE_y': results.MSE_y,
        'MSE_z': results.MSE_z,
...
def print_test_results(d):
    l = [
        ['', 'MAE (mm)', 'MAPE (mm)', 'MSE (mm^2)', 'RMSE (mm)'],
        ['x', f'{d["MAE_x"]:.3f}', f'{d["MAPE_x"]:.3f}', f'{d["MSE_x"]:.3f}', f'{np.sqrt(d["MSE_x"]):.3f}'],
        ['y', f'{d["MAE_y"]:.3f}', f'{d["MAPE_y"]:.3f}', f'{d["MSE_y"]:.3f}', f'{np.sqrt(d["MSE_y"]):.3f}'],
        ['z', f'{d["MAE_z"]:.3f}', f'{d["MAPE_z"]:.3f}', f'{d["MSE_z"]:.3f}', f'{np.sqrt(d["MSE_z"]):.3f}'],
        ['Position', f'{d["MAE_position"]:.3f}', f'{d["MAPE_position"]:.3f}', f'{d["MSE_position"]:.3f}', f'{np.sqrt(d["MSE_position"]):.3f}'],
        ['Amplitude', f'{d["MAE_amplitude"]:.3f}', f'{d["MAPE_amplitude"]:.3f}', f'{d["MSE_amplitude"]:.3f}', f'{np.sqrt(d["MSE_amplitude"]):.3f}'],
        ['Radius', f'{d["MAE_radius"]:.3f}', f'{d["MAPE_radius"]:.3f}', f'{d["MSE_radius"]:.3f}', f'{np.sqrt(d["MSE_radius"]):.3f}'],
    ]
    table = Table(l)
    table.write()
